...
module.exports = {
  async getEventivalBadges(email) {
    ...
    const response = await new Promise((resolve, reject) => {
      const req = https.get(`https://bo.eventival.com/poff/28th/en/ws/${process.env['EventivalWebToken']}/people/badges-for-login-email.xml?login_email=${email}`, function (res) {
        res.on('data', chunk => {
          dataString += chunk
        })
        res.on('end', () => {
