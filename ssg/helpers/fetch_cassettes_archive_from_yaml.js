...
const festivalEditionDirPath = path.join(strapiDataDirPath, 'FestivalEdition.yaml')
const STRAPIDATA_FESTIVAL_EDITIONS = yaml.load(fs.readFileSync(festivalEditionDirPath, 'utf8'))
...
if (CHECKPROGRAMMES) {
    ...
    if (cassettesWithOutProgrammes && cassettesWithOutProgrammes.length) {
        timer.log(__filename, `Archive cassettes with IDs ${cassettesWithOutProgrammes.join(', ')} have no programmes`)
    }
...
for (const lang of allLanguages) {
    ...
    timer.log(__filename, `Fetching ${DOMAIN} cassettes archive ${lang} data`)
    let allData = []
    ...
    if (slugMissingErrorNumber > 0) {
        timer.log(__filename, `Notification! Value of slug_en or slug_et missing for total of ${slugMissingErrorNumber} archive cassettes with ID's ${slugMissingErrorIDs.join(', ')}`)
    }
    if (cassettesWithOutFilms.length) {
        uniqueIDs = [...new Set(cassettesWithOutFilms)]
        timer.log(__filename, `ERROR! No films under archive cassettes with ID's ${uniqueIDs.join(', ')}`)
    }
    if (cassettesWithOutSpecifiedScreeningType.length) {
        uniqueIDs2 = [...new Set(cassettesWithOutSpecifiedScreeningType)]
        timer.log(__filename, `Skipped archive cassettes with IDs ${uniqueIDs2.join(', ')}, as none of screening types are ${whichScreeningTypesToFetch.join(', ')}`)
    }
...
function generateYaml(element, lang) {
    ...
    if (mapping[DOMAIN]) {
        let cassetteIndexTemplate = path.join(cassetteTemplatesDir, `cassette_${mapping[DOMAIN]}_archive_index_template.pug`)
        if (fs.existsSync(cassetteIndexTemplate)) {
            fs.writeFileSync(`${element.directory}/index.pug`, `include /_templates/cassette_templates/cassette_${mapping[DOMAIN]}_archive_index_template.pug`)
        } else {
            ...
            fs.writeFileSync(`${element.directory}/index.pug`, `include /_templates/cassette_templates/cassette_poff_archive_index_template.pug`)
        }
    }
}
...
function generateAllDataYAML(allData, lang) {
    ...
    fs.writeFileSync(path.join(fetchDir, `cassettes_archive.${lang}.yaml`), allDataYAML, 'utf8')
    timer.log(__filename, `Ready for building are ${allData.length} archive cassettes`)
    ...
    fs.writeFileSync(path.join(fetchDir, `search_films_archive.${lang}.yaml`), searchYAML, 'utf8')

    let filtersYAML = yaml.dump(sorted_filters, { 'noRefs': true, 'indent': '4' })
    fs.writeFileSync(path.join(fetchDir, `filters_films_archive.${lang}.yaml`), filtersYAML, 'utf8')
}
